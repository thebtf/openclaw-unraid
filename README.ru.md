...
docker restart OpenClaw
```
Оставьте **Config Migration** со значением по умолчанию `auto` и выполните одно обычное обновление образа. Если образ распознает точную поддерживаемую legacy или partial конфигурацию OpenClaw 2026.8.1, он сначала перенесёт побайтово точную частную временную копию и проверит этот кандидат через настоящий CLI OpenClaw. Только затем он создаст поблизости побайтово точную резервную копию, атомарно применит миграцию к реальной конфигурации и запустит шлюз. Текущая конфигурация не изменяется и не создаёт резервную копию. Неподдерживаемая конфигурация или кандидат, не прошедший проверку, остаётся побайтово без изменений, настоящая резервная копия не создаётся, а запуск отказывается продолжать. Обычный перезапуск идемпотентен и не создаёт вторую резервную копию.

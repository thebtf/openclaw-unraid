...
Если конфигурация не поддерживается/не проходит проверку кандидата либо upstream-проверка рабочего пространства выдаёт предупреждение или не может однозначно определить допустимый источник, запуск останавливается до изменения рабочего пространства. Затронутые файлы рабочего пространства остаются побайтово без изменений; резервная копия рабочего пространства, импорт SQLite и удаление источника не выполняются. Источники остаются для ручного восстановления, а управляемые записи отклоняются; устраните указанную в логе проблему идентификации источника вместо запуска широкого `doctor --fix`.
...
- `workspace-migration: refused` — upstream обнаружил предупреждение, некорректное/небезопасное состояние или неоднозначный источник. Legacy-источники рабочего пространства оставлены для ручного восстановления, а управляемые записи отклоняются; устраните указанную в логе идентификацию источника. Изменения рабочего пространства, резервная копия, импорт SQLite и удаление источника не выполнялись. Не запускайте широкий `doctor --fix`.

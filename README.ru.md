...
| Config Migration | Variable | Нет | `auto` | Оставьте `auto` для одного обычного обновления образа. Сохранённое legacy-значение `check` выдаёт предупреждение и работает как `auto`; для проверки без записи используйте только `dry-run`. См. [Обновление](#обновление). |
...
docker restart OpenClaw
```
Оставьте **Config Migration** со значением по умолчанию `auto` и выполните одно обычное обновление образа. Сначала образ обрабатывает только точную поддерживаемую legacy или partial **конфигурацию** OpenClaw 2026.8.1: он переносит побайтово точную частную временную копию, проверяет её настоящим CLI OpenClaw, затем создаёт поблизости побайтово точную резервную копию и атомарно применяет миграцию к реальной конфигурации. Текущая конфигурация не изменяется и не создаёт резервную копию.

Сбой кандидата конфигурации — это сбой миграции конфигурации, а не отказ миграции рабочего пространства: реальная конфигурация остаётся побайтово без изменений, резервная копия миграции конфигурации не создаётся, а управляемые записи останавливаются. Его ограниченная категория fatal:

```
[bootstrap] FATAL: existing OpenClaw config is invalid and is not a supported v2026.8.1 migration shape.
```

После миграции конфигурации и нативной проверки образ также автоматически проверяет только legacy-состояние настройки рабочих пространств отдельных агентов, которое распознаёт upstream-импортёр OpenClaw. Для каждого активного распознанного источника он проверяет побайтово точную резервную копию рядом с источником с именем `<active-basename>.openclaw-2026.8.1-pre-migration.bak` перед запуском upstream workspace-only import, выводит `workspace-migration: applied`, затем продолжает загрузку. Если legacy-состояния настройки рабочего пространства нет, он выводит `workspace-migration: no-op`; обычный перезапуск идемпотентен и не создаёт вторую резервную копию. Широкая команда `doctor --fix` не нужна и автоматически не запускается.
После существующей миграции и проверки конфигурации, а также миграции рабочего пространства, образ оставляет Gateway остановленным и запускает целевой импорт SQLite-сеансов OpenClaw для всех агентов. В проверенной миграции OpenClaw создавал манифест миграции и архивировал устаревшие файлы сеансов только после проверки. Импорт может занять несколько минут и уплотняет большие существующие базы данных SQLite.

Перед запуском Gateway образ выполняет целевую постпроверку dry-run для всех агентов и отказывается запускаться, если она не сообщает ноль устаревших целей и ноль проблем. Чтобы проверить то же состояние без импорта, выполните в контейнере эту команду диагностики только для чтения:

```bash
node /app/dist/index.js doctor --session-sqlite dry-run --session-sqlite-all-agents --json
```

Помощник выводит только `session-migration: no-op` или `session-migration: applied targets=<n> entries=<n> events=<n>`. Обычный перезапуск идемпотентен. Образ не запускает автоматически широкую команду `openclaw doctor`. Отдельные предупреждения плагинов или логирования не являются блокировкой `Legacy session store requires migration`.

`workspace-migration: refused` относится только к состоянию рабочего пространства: источник остаётся для ручного восстановления, а управляемые записи отклоняются. Некорректный или небезопасный источник, отклонённый на предварительной проверке, остаётся побайтово без изменений и не создаёт резервную копию рабочего пространства, импорт SQLite или удаление источника. Более поздний отказ (например, блокировка времени выполнения или предупреждение после проверенных резервных копий) не удаляет уже проверенные резервные копии; устраните указанную в логе проблему идентификации источника вместо запуска широкого `doctor --fix`.

```
workspace-migration: refused
```

В старых контейнерах может сохраниться `OPENCLAW_CONFIG_MIGRATION=check`. Это совместимо: будет выдано предупреждение об устаревании, после чего режим работает точно как `auto`, поэтому обычное обновление завершится. Для явной проверки без записи в реальную конфигурацию и без резервной копии установите `OPENCLAW_CONFIG_MIGRATION=dry-run`; он выведет узкий план и откажется запускаться. Затем перезапустите контейнер с `auto`, это значение по умолчанию. Устанавливайте `OPENCLAW_CONFIG_MIGRATION=apply-v2026.8.1` только для явного расширенного применения при восстановлении.
...
- `FATAL: OPENCLAW_CONFIG_MIGRATION='...' is invalid` — используйте `auto` (значение по умолчанию), `dry-run` или `apply-v2026.8.1`; сохранённое `check` принимается только как устаревший совместимый псевдоним `auto`.
- `existing OpenClaw config is invalid and is not a supported v2026.8.1 migration shape` — конфигурация осталась побайтово без изменений, резервная копия миграции не создана. Сохраните `openclaw.json` и восстановите его вручную. Не запускайте широкую команду `doctor --fix` как автоматический шаг обновления.
- `candidate migration failed native OpenClaw validation` — реальная конфигурация осталась побайтово без изменений, резервная копия миграции не создана; сохраните `openclaw.json` и восстановите его вручную.
- `workspace-migration: refused` — helper рабочего пространства завершился с отказом до запуска шлюза: не прошли небезопасный или неоднозначный источник, проверка runtime record/hash/path, блокировка, резервное копирование/импорт либо финальная верификация. Проверьте предшествующую ограниченную строку `FATAL:`; сохраните все legacy-источники и любые существующие проверенные резервные копии. Управляемые записи остаются отклонёнными. Не запускайте широкий `doctor --fix` автоматически.

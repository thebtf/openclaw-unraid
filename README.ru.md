...
Перед обновлением сделайте и проверьте резервную копию OpenClaw Data. Само обновление образа обнаруживает сохранённую конфигурацию, которой нужна миграция, но никогда не применяет изменения миграции без opt-in. `OPENCLAW_CONFIG_MIGRATION` — расширенная переменная окружения со значением по умолчанию `check`. Для недействительного существующего `openclaw.json` режим `check` запускает узкий мигратор. Когда он выводит не содержащий значений план затрагиваемых путей, запуск завершается без изменения конфига и создания резервной копии. Точный результат `already migrated` при всё ещё красной нативной проверке не поддерживается и не допускает управляемых записей.

Чтобы один раз явно разрешить миграцию v2026.8.1, задайте `OPENCLAW_CONFIG_MIGRATION=apply-v2026.8.1` в Unraid **Edit Container** или в окружении Compose и запустите контейнер. До записи мигратор создаёт рядом резервную копию с меткой времени `openclaw.json.v2026.8.1-backup-*` и выводит точный путь к ней. Проверьте этот путь, верните `OPENCLAW_CONFIG_MIGRATION` к `check` и перезапустите контейнер обычным образом. Для последующего запуска с действительным конфигом токен не нужен. После проверки пути к резервной копии верните `check`: последующий образ принимает только свой токен с версией и отклоняет устаревшие значения.

В логах строка `[bootstrap] existing config needs OpenClaw v2026.8.1 migration`, затем `dry run: planned changed paths` и FATAL с инструкцией по opt-in означают ожидаемый безопасный отказ. `FATAL: existing OpenClaw config is invalid but the v2026.8.1 migrator reports already migrated` означает, что узкая миграция не может исправить этот недействительный конфиг; не задавайте токен apply. Не используйте полный `openclaw doctor --fix` как штатное восстановление после обновления. Это ручной инструмент диагностики после сохранения резервной копии; entrypoint никогда не запускает его автоматически.

Если контейнер остановлен и не может запустить мигратор образа, используйте ручной вариант из этого репозитория. Найдите путь хоста **OpenClaw Data** в шаблоне Unraid. Не вставляйте значения конфигурации в команду. Сначала выполните `python3 scripts/migrate-openclaw-2-config.py --config <путь-хоста-OpenClaw-Data>/openclaw.json`; по умолчанию это пробный запуск. После проверки вывода добавьте `--apply`, чтобы выполнить миграцию. Скрипт создаёт рядом резервную копию с меткой времени, побайтно идентичную исходному файлу, и выводит только затронутые пути.
...
- `FATAL: OPENCLAW_CONFIG_MIGRATION='...' is invalid` — используйте `check` или одноразовый токен `apply-v2026.8.1`.
- `FATAL: existing OpenClaw config is invalid.` — режим `check` по умолчанию вывел план путей и отказался от записей; следуйте инструкции по обновлению выше перед opt-in.
- `FATAL: existing OpenClaw config is invalid but the v2026.8.1 migrator reports already migrated` — эта недействительная конфигурация не поддерживается узкой миграцией. Не задавайте токен apply; используйте ручную диагностику после сохранения конфига.

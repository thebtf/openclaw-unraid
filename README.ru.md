...
| Config Migration | Variable | Нет | `auto` | При обновлении образа переносит только поддерживаемые legacy или partial конфигурации OpenClaw 2026.8.1. См. [Обновление](#обновление). |
...
docker restart OpenClaw
```
Оставьте **Config Migration** со значением по умолчанию `auto` и один раз обновите образ обычным способом. Если образ распознает точную поддерживаемую legacy или partial конфигурацию OpenClaw 2026.8.1, он сначала создаст поблизости побайтово точную резервную копию, атомарно применит миграцию и затем запустит шлюз. Текущая конфигурация не изменяется и не создаёт резервную копию. Неоднозначная или неподдерживаемая конфигурация остаётся побайтово без изменений, резервная копия не создаётся, а запуск отказывается продолжать и выводит следующее сообщение о восстановлении. Обычный перезапуск идемпотентен и не создаёт вторую резервную копию.
...
Для `auto` не нужна широкая команда `doctor --fix` или ручной помощник миграции. Только для диагностики установите `OPENCLAW_CONFIG_MIGRATION=check`. Этот режим ничего не записывает и не создаёт резервную копию. Затем перезапустите контейнер с `OPENCLAW_CONFIG_MIGRATION=auto`, это значение по умолчанию. Устанавливайте `OPENCLAW_CONFIG_MIGRATION=apply-v2026.8.1` только для явного применения поддерживаемой миграции при восстановлении.
...
- `FATAL: OPENCLAW_CONFIG_MIGRATION='...' is invalid` — используйте `auto` (значение по умолчанию), `check` или `apply-v2026.8.1`.

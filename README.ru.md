...
После миграции конфигурации и нативной проверки образ также автоматически проверяет только legacy-состояние настройки рабочих пространств отдельных агентов, которое распознаёт upstream-импортёр OpenClaw. Для каждого активного распознанного источника он проверяет побайтово точную резервную копию рядом с источником с именем `<active-basename>.openclaw-2026.8.1-pre-migration.bak` перед запуском upstream workspace-only import, выводит `workspace-migration: applied`, затем штатно запускает шлюз. Если legacy-состояния настройки рабочего пространства нет, он выводит `workspace-migration: no-op`; обычный перезапуск идемпотентен и не создаёт вторую резервную копию. Широкая команда `doctor --fix` не нужна и автоматически не запускается.
...
- `workspace-migration: refused` — upstream обнаружил предупреждение, некорректное/небезопасное состояние или неоднозначный источник. Legacy-источники рабочего пространства оставлены для ручного восстановления, а управляемые записи отклоняются; устраните указанную в логе идентификацию источника. Некорректный или небезопасный источник, отклонённый на предварительной проверке, не изменён и не создаёт резервную копию рабочего пространства, импорт SQLite или удаление источника. Более поздний отказ может сохранить уже проверенные резервные копии. Не запускайте широкий `doctor --fix`.

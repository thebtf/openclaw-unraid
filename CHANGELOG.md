...
- **Persisted-config migration.** `OPENCLAW_CONFIG_MIGRATION` defaults to `auto`, so one normal image update migrates only an exact supported OpenClaw 2026.8.1 legacy or partial configuration shape. Before a real backup or write, the image transforms a byte-identical private temporary copy with the narrow migrator and validates that candidate through the native OpenClaw CLI; only then does it write a byte-exact adjacent backup and apply the real migration atomically. Current configuration is a no-op. Unsupported or candidate-invalid configuration remains byte-identical with no real backup. Persisted `check` is now a deprecated compatibility alias for `auto`; `dry-run` is the explicit no-write/no-backup inspection mode, and `apply-v2026.8.1` remains an advanced explicit apply. No routine `doctor --fix` command or manual helper is required.
- **Legacy workspace setup-state migration.** One normal image update now automatically detects only per-agent legacy workspace setup state that the upstream OpenClaw importer recognizes. The image verifies byte-exact adjacent backups named `<active-basename>.openclaw-2026.8.1-pre-migration.bak` for every active recognized source before the upstream workspace-only import, logs `workspace-migration: applied`, then starts normally; it logs `workspace-migration: no-op` when no legacy state remains, so a restart creates no second backup. `workspace-migration: refused` is workspace-only: it leaves sources for manual recovery and refuses managed writes. A malformed or unsafe source rejected during preflight is unchanged and creates no workspace backup, SQLite import, or source removal; a later refusal may retain already verified backups. Broad `doctor --fix` is neither required nor run automatically.
- **Legacy session store migration.** Fixes `Legacy session store requires migration` by running OpenClaw's focused all-agent session SQLite import before Gateway startup, then requiring a zero-target, zero-issue dry-run postflight. The upstream import writes its migration manifest and archives legacy session files after validation; normal restarts are idempotent. The image does not run broad `openclaw doctor` automatically.

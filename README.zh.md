...
| Config Migration | Variable | 否 | `auto` | 镜像更新时仅迁移受支持的 OpenClaw 2026.8.1 legacy 或 partial 配置形态。参见[更新升级](#updating)。 |
...
docker restart OpenClaw
```
保留 **Config Migration** 的默认值 `auto`，然后按正常方式更新一次镜像。如果镜像识别出确切受支持的 OpenClaw 2026.8.1 legacy 或 partial 配置形态，它会先在相邻位置创建逐字节一致的备份，以原子方式应用迁移，然后启动网关。当前配置不会被修改，也不会创建备份。含糊或不受支持的配置会保持逐字节不变，不创建备份，启动会拒绝继续并输出以下恢复说明。正常重启是幂等的，不会创建第二个备份。
...
`auto` 不需要运行宽泛的 `doctor --fix` 命令或手动迁移辅助工具。仅在诊断时，将 `OPENCLAW_CONFIG_MIGRATION=check` 设为只检查。该模式不会写入，也不会创建备份。随后以默认值 `OPENCLAW_CONFIG_MIGRATION=auto` 重启容器。仅在恢复期间需要显式应用受支持的迁移时，将 `OPENCLAW_CONFIG_MIGRATION=apply-v2026.8.1` 设为该值。
...
- `FATAL: OPENCLAW_CONFIG_MIGRATION='...' is invalid` —— 使用 `auto`（默认值）、`check` 或 `apply-v2026.8.1`。
